 ...
 */
import { actionInputSchemas, createActivityInput, createNoteInput, moveStageInput, summarizeDealInput, draftFollowupEmailInput, weeklyReportInput, } from "./schemas.js";
...
export function isAllowlistedAction(action) {
...
export async function executeAction(ctx, actionType, input) {
    ...
    switch (actionType) {
        case "summarize_deal":
            return runSummarizeDeal(ctx, parsed.data);
        case "draft_followup_email":
            return runDraftFollowupEmail(ctx, parsed.data);
        case "create_note":
            return runCreateNote(ctx, parsed.data);
        case "create_activity":
            return runCreateActivity(ctx, parsed.data);
        case "move_stage":
            return runMoveStage(ctx, parsed.data);
        case "weekly_report":
            return runWeeklyReport(ctx, parsed.data);
        default:
            return { ok: false, error: `Unimplemented action: ${actionType}` };
    }
}
async function runSummarizeDeal(ctx, input) {
    try {
...
async function runDraftFollowupEmail(ctx, input) {
    try {
        const deal = await ctx.pipedrive.getDeal(input.dealId);
        if (!deal)
            return { ok: false, error: "Deal not found" };
...
async function runCreateNote(ctx, input) {
    if (!ctx.confirmedActionRequestId) {
...
async function runCreateActivity(ctx, input) {
    if (!ctx.confirmedActionRequestId) {
...
async function runMoveStage(ctx, input) {
    if (!ctx.confirmedActionRequestId) {
...
async function runWeeklyReport(ctx, input) {
    try {
...
export function validateActionInput(actionType, input) {
    if (!isAllowlistedAction(actionType)) {
