...
import { executeAction, ACTION_METADATA } from "../actions/index.js";
import { canRequestActions } from "../auth/index.js";
import { getProductCustomNum, getAvailableRoomsFieldKey, getStockQuantityFieldKey, getRoomTypeFieldKey, getDepartureDateFieldKey, getProductDateYearMonth, getProductDateSortable, PRODUCTS_FETCH_LIMIT, } from "../lib/product-fields.js";
const messageBody = z.object({
    sessionId: z.string().nullish(),
    message: z.string().min(1).max(10000),
});
export async function chatRoutes(fastify, deps) {
    const ctx = { pipedrive: deps.pipedrive };
    fastify.post("/api/chat/message", async (req, reply) => {
        ...
        if (plan.intent === "query") {
            ...
            await deps.db.chatMessage.create({
                data: {
                    ...
                    metadata: queryResult.metadata ? queryResult.metadata : undefined,
                },
            });
            return reply.send({
                sessionId,
        ...
        if (plan.intent === "action" && plan.actionType && plan.input != null) {
            ...
            if (meta.requiresConfirmation && meta.isWrite) {
                ...
                const text = (plan.humanSummary ?? "הצעה לפעולה") +
    ...
    fastify.get("/api/chat/sessions", async (req, reply) => {
    ...
    fastify.get("/api/chat/sessions/:sessionId/messages", async (req, reply) => {
        const r = req;
        const session = await deps.db.chatSession.findFirst({
...
function isLeadSourcesQuery(msg) {
    const t = msg.trim();
...
function parseAvailableRoomsAbove(msg) {
    const t = msg.trim();
...
function isAvailableRoomsAboveQuery(msg) {
    return parseAvailableRoomsAbove(msg) != null;
}
function isProductsInventoryQuery(msg) {
    const t = msg.trim().toLowerCase();
...
function isTotalAvailableRoomsQuery(msg) {
    const t = msg.trim();
    return (/חשב\s*(?:לי\s*)?כמה\s*חדרים\s*פנויים|כמה\s*חדרים\s*פנויים\s*יש\s*[?.]?$/i.test(t) ||
...
function isAvailableRoomsQuery(msg) {
    const t = msg.trim();
 ...
 */
function parseDateFilterFromMessage(msg) {
    const t = msg.trim();
    let year;
    let month;
...
function isDateFilteredInventoryQuery(msg) {
    const t = msg.trim();
    const hasRoomsOrStock = /חדרים\s*(?:פנויים|נותרו|נשארו)|כמה\s*יש\s*במלאי|מלאי\s*ב/i.test(t) ||
 ...
 */
function formatInventoryGrouped(products, opts) {
    ...
    const lineSep = "─────────────────────────────────────";
    let out = blocks.join("\n\n" + lineSep + "\n\n");
    if (products.length > slice.length)
        out += "\n\n" + lineSep + "\n\n... ועוד " + (products.length - slice.length) + " פריטים";
    return out;
}
/** שאלה שקשורה לנתונים/מאגר – נחזיר סיכום עדכני */
function isDataRelatedQuery(msg) {
    const t = msg.trim();
    return (/נתונים|מאגר|דוח|מצב|כמה\s+יש|מה\s+המצב|סטטוס|סיכום\s+כללי|פייפדרייב|pipedrive|מידע\s+עדכני/i.test(t) ||
...
function isStageCountQuery(msg) {
    const t = msg.trim();
    if (/טרום\s*שיחה|ליד\s*טרום\s*שיחה/i.test(t))
        return { stageName: "טרום שיחה", openOnly: /עסקאות\s*פתוחות|פתוחות\s*ב/i.test(t) };
    ...
    if (m) {
        ...
        if (name && name.length < 50)
            return { stageName: name, openOnly: /עסקאות\s*פתוחות|פתוחות\s*ב/i.test(t) };
    }
    return null;
}
/** לידים תקועים יותר מ־X ימים */
function isStuckDealsQuery(msg) {
    const t = msg.trim();
    if (!/תקוע|ללא\s+פעילות|נתקע/i.test(t))
        return null;
    const week = t.match(/יותר\s*מ(?:־|\s)?(\d+)\s*יום/i);
    if (week)
        return parseInt(week[1], 10) || 7;
    const days = t.match(/(\d+)\s*יום/i);
    if (days)
        return parseInt(days[1], 10);
    if (/שבוע|week/i.test(t))
        return 7;
    return 7;
}
function dealAddTime(deal) {
    const raw = deal;
...
function getStartOfTodayIsraelMs() {
    const now = new Date();
...
function getStartOfMonthIsraelMs() {
...
function isFunnelQuery(msg) {
    const t = msg.trim();
    return /משפנמה|פונל|בכל\s*שלב|לידים\s*לפי\s*שלב|עסקאות\s*לפי\s*שלב|דוח\s*משפנמה/i.test(t);
}
/** עסקאות שזכינו / נסגרו בהצלחה / הפסדנו */
function isWonLostQuery(msg) {
    const t = msg.trim();
    if (/זכינו|נסגר\s*בהצלחה|won|סגורות\s*בהצלחה|נצחונות|החודש\s*זכינו|נסגרו\s*החודש/i.test(t))
        return "won";
    if (/הפסדנו|הפסד|lost|איבדנו/i.test(t))
        return "lost";
    if (/פתוחות|open|פעילות\s*פתוחות|סה"כ\s*עסקאות\s*פעילות/i.test(t))
        return "open";
    return null;
}
/** שווי עסקאות */
function isValueQuery(msg) {
    const t = msg.trim();
    return /שווי|ערך|סה"כ\s*כסף|כמה\s*כסף|value|סיכום\s*כספי/i.test(t);
}
/** דוח מנהלים / סיכום מנהלים */
function isManagerSummaryQuery(msg) {
    const t = msg.trim();
...
function isByOwnerQuery(msg) {
    const t = msg.trim();
    if (/דוח\s*לפי\s*בעלים|עסקאות\s*לפי\s*בעלים|לידים\s*לפי\s*בעלים/i.test(t))
        return true;
    const m = t.match(/עסקאות\s*(?:של|)\s*(.+?)(?:\?|$)/i) || t.match(/לידים\s*(?:של|)\s*(.+?)(?:\?|$)/i);
    if (m)
        return { ownerName: m[1].trim() };
    return false;
}
/** לפי חודש – "כמה נסגרו החודש", "חודש שעבר" */
function isByMonthQuery(msg) {
    const t = msg.trim();
    if (/החודש|חודש\s*נוכחי|this\s*month|נסגרו\s*החודש/i.test(t))
        return "this";
    if (/חודש\s*שעבר|החודש\s*שעבר|last\s*month/i.test(t))
        return "last";
    return null;
}
/** לפי פייפליין – "כמה לידים בכל פייפליין", "דוח לפי פייפליין" */
function isByPipelineQuery(msg) {
    const t = msg.trim();
...
function parseConversionRateRepName(msg) {
    const t = msg.trim();
    const m = t.match(/אחוז\s*המרה\s*(?:של\s*)?(?:נציג\s*)?([^\s?.]+(?:\s+[^\s?.]+)*)/i) ||
...
function isWonSalesThisMonthQuery(msg) {
    const t = msg.trim();
...
function isStuckTwoWeeksQuery(msg) {
    const t = msg.trim();
...
function isNewDealsOpenedQuery(msg) {
    const t = msg.trim();
...
function isMonthlySalesGrowthQuery(msg) {
    const t = msg.trim();
    return /אחוז\s*גידול|גידול\s*מחודש\s*לחודש|מונט\s*למונט|growth\s*מכירות/i.test(t);
}
/** נציג עם Win Rate הגבוה / שווי תיק הגבוה */
function isRepLeaderboardQuery(msg) {
    const t = msg.trim();
    if (/מי\s*.*\s*win\s*rate|נציג\s*.*\s*win\s*rate|הכי\s*הרבה\s*זכיות|אחוז\s*המרה\s*גבוה|מי\s*הנציג\s*הכי\s*מוצלח|נציג\s*עם\s*הכי\s*הרבה\s*זכיות|מי\s*מוביל\s*באחוז\s*זכייה/i.test(t))
        return "winrate";
    if (/נציג\s*שווי\s*תיק|שווי\s*תיק\s*גבוה|מנהל\s*הכי\s*הרבה\s*שווי|פייפליין\s*גבוה\s*נציג|מי\s*מנהל\s*הכי\s*הרבה\s*עסקאות|נציג\s*עם\s*התיק\s*הגדול/i.test(t))
        return "pipeline";
    return null;
}
/** סיבת הפסד (Lost Reason) נפוצה */
function isLostReasonQuery(msg) {
    const t = msg.trim();
...
function isDealLifecycleQuery(msg) {
    const t = msg.trim();
...
function matchIntentByKeywords(msg) {
    const t = msg.trim().toLowerCase();
    if ((/מכירות|מכרנו|סגרנו|הכנסות|זכינו|נסגרו/.test(t) && /חודש|הנוכחי/.test(t)) || (/כמה|מה\s*סך|מהו\s*סך/.test(t) && /מכירות|הכנסות/.test(t) && /חודש/.test(t)))
        return "won_sales_month";
    if (/אחוז\s*זכייה|win\s*rate|וין\s*רייט|אחוז\s*הצלחה|כמה\s*אחוז\s*זכינו/.test(t) && !/נציג|מי\s*הנציג|לפי\s*נציג/.test(t))
        return "team_win_rate";
    if (/שווי\s*צינור|שווי\s*פתוחות|פייפליין|כמה\s*כסף\s*בצינור|שווי\s*העסקאות\s*הפתוחות/.test(t))
        return "pipeline_value";
    if (/ממוצע\s*עסקה|שווי\s*ממוצע|ערך\s*ממוצע|גודל\s*עסקה\s*ממוצע|average\s*deal/.test(t))
        return "average_deal_value";
    if (/כמה\s*לידים|כמה\s*עסקאות/.test(t) && /היום|השבוע|אתמול/.test(t))
        return "deals_today_week";
    if (/מקורות?\s*הגעה|מאיזה\s*מקור|מקור\s*לידים/.test(t))
        return "lead_sources";
    if (/דוח\s*מנהל|סיכום\s*מנהל|מבט\s*על|תמונת\s*מצב/.test(t))
        return "manager_summary";
    if (/אחוז\s*המרה\s*של|המרה\s*של\s*נציג/.test(t))
...
async function runQuery(ctx, message, plan, opts) {
    ...
    if (isGreeting(message)) {
        return {
            text: "שלום! איך אוכל לעזור? דוגמאות לשאלות:\n" +
    ...
    function pipedriveErrorText(e) {
        const msg = e instanceof Error ? e.message : String(e);
        if (msg.includes("401") || msg.includes("Unauthorized"))
            return "חיבור ל-Pipedrive נכשל: הטוקן לא תקף. עדכן את PIPEDRIVE_API_TOKEN ב-.env מההגדרות ב-Pipedrive.";
        if (msg.includes("403"))
            return "אין הרשאה לגשת לנתונים. בדוק את הטוקן ב-Pipedrive.";
        if (msg.includes("404"))
            return "כתובת ה-API לא נמצאה. וודא ש-PIPEDRIVE_DOMAIN תקין או השאר ריק (נשתמש ב-api.pipedrive.com).";
        if (msg.includes("429"))
            return "יותר מדי בקשות ל-Pipedrive. חכה רגע ונסה שוב.";
                ...
                for (const key of reasonKeys) {
                    ...
                    if (r != null) {
                        if (typeof r === "object" && r !== null) {
                            ...
                            else if ("id" in r && r.id != null)
                                label = `סיבה #${r.id}`;
                            else
                                label = "ללא סיבה (שדה אובייקט)";
                        }
                        else
                            label = String(r);
                        break;
    ...
    if (isLeadToDealConversionQuery(message)) {
        try {
            const deals = await ctx.pipedrive.listDeals(MAX_DEALS_FETCH);
            const withSource = deals.filter((d) => d.lead_source_id != null);
            const won = deals.filter((d) => (d.status ?? "") === "won");
            const totalWithSource = withSource.length;
            const wonWithSource = won.filter((d) => d.lead_source_id != null);
    ...
    if (isFollowUpSourcesAfterYesterday) {
        try {
            ...
            const totalFromSources = dealsYesterday.filter((d) => d.lead_source_id != null).length;
            return {
    ...
    if (isAvailableRoomsAboveQuery(message)) {
        ...
        if (minRooms == null)
            return { text: "לא זיהיתי כמות. נסה: כמה חדרים פנויים מעל 5" };
        try {
            ...
            const above = products.filter((p) => getProductCustomNum(p, availableRoomsKey) >= minRooms);
            const totalRooms = above.reduce((sum, p) => sum + getProductCustomNum(p, availableRoomsKey), 0);
            const lineSep = "─────────────────────────────────────";
            const productLines = above.slice(0, 100).map((p) => {
                const name = (p.name ?? "ללא שם").trim() || "ללא שם";
                const x = getProductCustomNum(p, availableRoomsKey);
                return "\u202B" + name + "\u202C\nכמות חדרים פנויים : " + x;
            });
    ...
    if (stageCount) {
        try {
            ...
            const stage = stages.find((s) => (s.name ?? "").toLowerCase().replace(/\s+/g, " ").includes(nameLower) || nameLower.includes((s.name ?? "").toLowerCase()));
            if (!stage) {
                return {
    ...
    if (isManagerSummaryQuery(message) || matchIntentByKeywords(message) === "manager_summary") {
        try {
            ...
            const funnelLines = stages
                ...
                .join("\n");
            const text = `נתונים עדכניים מ-Pipedrive – דוח מנהלים:\n\n` +
    ...
    if (isConversionRateQuery(message) || matchIntentByKeywords(message) === "conversion_rate") {
        ...
        try {
            ...
            const user = users.find((u) => (u.name ?? "").toLowerCase().includes(nameLower) ||
    ...
    if (byMonth) {
        try {
            ...
            const wonInPeriod = won.filter((d) => dealWonTime(d) >= monthStartMs && dealWonTime(d) <= monthEndMs);
        ...
        if (dateFilter && (dateFilter.year != null || dateFilter.month != null)) {
            try {
                ...
                const filtered = products.filter((p) => {
                    ...
                    if (!dm)
                        return false;
                    if (dateFilter.year != null && dm.year !== dateFilter.year)
                        return false;
                    if (dateFilter.month != null && dm.month !== dateFilter.month)
                        return false;
                    return true;
                });
                const totalRooms = filtered.reduce((sum, p) => sum + getProductCustomNum(p, availableRoomsKey), 0);
                const label = dateFilter.month != null && dateFilter.year != null
                ...
                const body = formatInventoryGrouped(filtered.map((p) => p), { availableRoomsKey, stockKey, roomTypeKey, departureKey, maxGroups: 50, maxPerGroup: 80 });
    ...
    if (isTotalAvailableRoomsQuery(message)) {
        try {
            ...
            if (!availableRoomsKey) {
                return {
                    text: "לא נמצא שדה 'חדרים פנויים' בהגדרות שדות המוצר ב-Pipedrive. וודא שיש שדה מותאם בשם 'חדרים פנויים' (או 'Available rooms') במוצרים.",
                };
            }
            const total = products.reduce((sum, p) => sum + getProductCustomNum(p, availableRoomsKey), 0);
            return {
    ...
    if (isAvailableRoomsQuery(message)) {
        try {
            ...
            if (!availableRoomsKey) {
                return {
                    text: "לא נמצא שדה 'חדרים פנויים' בהגדרות שדות המוצר ב-Pipedrive. וודא שיש שדה מותאם בשם 'חדרים פנויים' (או 'Available rooms') במוצרים, ושהטוקן יכול לקרוא שדות מוצר.",
                };
            }
            const withAvailable = products.filter((p) => getProductCustomNum(p, availableRoomsKey) > 0);
            ...
            const roomTypeKey = getRoomTypeFieldKey(fields);
            const departureKey = getDepartureDateFieldKey(fields);
            const body = formatInventoryGrouped(withAvailable.map((p) => p), { availableRoomsKey, stockKey, roomTypeKey, departureKey, maxGroups: 50, maxPerGroup: 80 });
            return {
    ...
    if (isProductsInventoryQuery(message)) {
        try {
            ...
            const body = formatInventoryGrouped(products.map((p) => p), { availableRoomsKey, stockKey, roomTypeKey, departureKey, maxGroups: 50, maxPerGroup: 80 });
    ...
    try {
        ...
        return {
            text: `לפי מה שהבנתי – הנה סיכום הנתונים במערכת:\n• עסקאות: ${deals.length}\n• מוצרים: ${products.length}\n\n` +
        ...
        return {
            text: "הנה סיכום: אפשר לשאול בכל ניסוח על מכירות, Win Rate, שווי צינור, לידים, מקורות הגעה, דוח מנהלים, רשימת מלאי – אענה לפי הכוונה.",
        };
    }
}
