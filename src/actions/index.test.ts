...
function ctx(confirmedId?: string): ActionContext {
  return {
    pipedrive: mockPipedrive as unknown as ActionContext["pipedrive"],
...
describe("allowlist", () => {
  ...
  it("executeAction returns error for unknown action", async () => {
    const result = await executeAction(ctx(), "unknown_action" as never, {});
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error).toContain("Unknown action");
  });

  it("validateActionInput rejects unknown action", () => {
    const v = validateActionInput("invalid" as never, {});
    expect(v.success).toBe(false);
    if (!v.success) expect(v.error).toContain("Unknown action");
  });
});
