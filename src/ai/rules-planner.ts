...
export function rulesBasedPlan(message: string): PlanOutput {
...
export function parseStructuredPlan(json: string): PlanOutput | null {
  try {
    const parsed = JSON.parse(json) as PlanOutput;
    if (parsed.intent === "action" && parsed.actionType) {
      if (!isAllowlistedAction(parsed.actionType)) return null;
      return parsed;
    }
    if (parsed.intent === "query") return parsed;
    return null;
  } catch {
    return null;
  }
